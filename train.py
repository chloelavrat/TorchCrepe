...
from utils import frequency_to_activation
...
def epoch_step(model, audio, labels, sr, device):
    ...
    labels_activations = frequency_to_activation(labels)

    # compute model activations
    model_activations = model.compute_activation(audio, sr).to(device)
