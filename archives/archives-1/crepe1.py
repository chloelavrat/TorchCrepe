...
class ConvBlock(nn.Module):
    def __init__(self, f, w, s, in_channels):
        ...
        self.conv2d = nn.Conv2d(in_channels=in_channels,
                                out_channels=f, kernel_size=(w, 1), stride=s)
        self.relu = nn.ReLU()
...
class CREPE(nn.Module):
    def __init__(self, model_capacity="full"):
        ...
        for i in range(len(self.layers)):
            f, w, s, in_channel = filters[i +
                                          1], widths[i], strides[i], filters[i]
    ...
    def load_weight(self, model_capacity):
        ...
        self.load_state_dict(torch.load(os.path.join(
            package_dir, filename), map_location=torch.device(device)))
    ...
    def process_file(self, file, output=None, viterbi=False, center=True, step_size=10, save_plot=False, batch_size=128):
        ...
        time, frequency, confidence, activation = [
            t.cpu().numpy() for t in (time, frequency, confidence, activation)]

        f0_file = os.path.join(output, os.path.basename(
            os.path.splitext(file)[0])) + ".f0.csv"
        f0_data = torch.stack([torch.tensor(time), torch.tensor(
            frequency), torch.tensor(confidence)], dim=1)
        ...
        if save_plot:
            ...
            plot_file = os.path.join(output, os.path.basename(
                os.path.splitext(file)[0])) + ".activation.png"
